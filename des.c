...
void write_bytes_hex(FILE *file, unsigned char *text, int nbytes)
{
    for (size_t i = 0; i < nbytes; i++)
    {
...
{
    printf("\nNOTE: Make sure your key is 64 bits long max\n\n");
    ...
    if (argv[1] == NULL || argv[2] == NULL || argv[3] == NULL)
    {
        ...
        printf("\nExample usage: DES.exe (encrypt|decrypt) [KEY_FILE TEXT_FILE]\n\n");
        return -1;
    }
    ...
    else
    {
        ...
        printf("\nExample usage: DES (encrypt|decrypt) [KEY_FILE TEXT_FILE]\n\n");
        return -1;
    }

    FILE *input_key = fopen(argv[2], "rb");
    FILE *input_text = fopen(argv[3], "rb");
    FILE *output_text;
    FILE *output_hex;
    ...
    if (mode)
    {
        output_text = fopen("encrypted.txt", "wb");
        output_hex = fopen("hex.txt", "wb");
    }
    else
    {
        output_text = fopen("decrypt.txt", "wb");
    }
    ...
    while (1)
    {
        fread(plaintext, sizeof plaintext, 1, input_text);
        if (feof(input_text))
            break;
        ...
        if (mode)
        {
            ...
            write_bytes_hex(output_hex, ciphertext, 8);
        }
        else
    ...
    {
        printf("\nEnd Encryption....\n");
        printf("Output Files Names are: 'encrypted.txt'\n");
        printf("                        'hex.txt'\n");
    }
    else
    {
        printf("\nEnd Decryption....\n");
        printf("Output File Name is: 'decrypt.txt'\n");
    }
    fclose(input_key);
    fclose(input_text);
    fclose(output_text);
